...
def create_parking():
    ...
    return (
        jsonify({"id": new_parking.id, "message": "Parking created successfully"}),
        201,
    )
...
def exit_parking():
    ...
    if not parking_log:
        return (
            jsonify({"error": "Client is not on this parking or has already exited"}),
            400,
        )
    ...
    return (
        jsonify({"message": "Client exited parking successfully, payment processed"}),
        200,
    )
